...
sp_500_daily_returns_tbl <- sp_500_prices_tbl %>% 
    ...
    # group by company and get 1 day lag
    group_by(symbol) %>% 
    mutate(adjusted_lag_1 = lag(adjusted, n = 1)) %>% 
    ungroup() %>% 
    
    # Handle NA
    filter(!is.na(adjusted_lag_1)) %>% 
    
    # Calculate Pct Return
    mutate(diff_1 =  adjusted - adjusted_lag_1) %>% 
    mutate(pct_return = diff_1 / adjusted_lag_1) %>% 
...
umap_kmeans_results_tbl <- k_means_obj %>% 
  ...
  left_join(umap_results_tbl, by = c("symbol" = "symbol")) %>% 
