...
**ANSWER:** Looks like the best time of the year is spring/summer

```{r}

bike_orderlines_tbl %>%
    
    # select columns and create month column
    select(order_date, total_price) %>% 
    mutate(month = month(order_date, label = T, abbr = F)) %>% 
    
    # group by month and summarize to get sales
    group_by(month) %>% 
...
**ANSWER:** That bikes with this feature are high value bicycles relative to those that don't

```{r}

bike_orderlines_tbl %>% 
    
    # select columns and create flag
    select(model, total_price) %>%
    mutate(black_inc = model %>% str_to_lower() %>% str_detect("black inc")) %>% 
    
    # group by and summarize median sales
    group_by(black_inc) %>% 
    summarize(median_price = median(total_price)) %>% 
    ungroup() %>% 
    
    # format output
    mutate(median_price = scales::dollar(median_price, largest_with_cents = 1e+05)) %>% 
...
**ANSWER:** That bikes with this feature in general cost on average the same as all other bikes.
...
bike_orderlines_tbl %>% 
    ...
    # group by and summarize median sales
    group_by(ultegra) %>% 
    summarize(sales = median(total_price)) %>% 
    ungroup() %>% 
    
    # format output
    mutate(sales = scales::dollar(sales, largest_with_cents = 1e+05)) %>% 
    set_names(names(.) %>% str_replace("_", " ") %>% str_to_title())
...
**ANSWER:** In general, bikes with the disc feature are priced lower than other bikes by this maker.
...
**ANSWER:** Wide range of prices within model-bases. Appears to be lower tier and upper tier.
